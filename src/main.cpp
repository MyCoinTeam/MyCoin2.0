...
// Copyright (c) 2014 The MyCoin2 developers
...
uint256 hashGenesisBlock("0xeb4ac0ee05395f1ab3e8bd64fb01ddd9d583374047c318a9d445d8b31281afac");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MyCoin2: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MyCoin2 Signed Message:\n";
...
{
    // MyCoin2: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight==1) return 870000000 * COIN + nFees;
	if (nHeight>=2 && nHeight<= 3153600000) return 0.003 * COIN + nFees;
	if (nHeight>=3153600001 ) return 0.00001 * COIN + nFees;
}

static const int64 nTargetTimespan = 100 * 365 * 24 * 60 * 60; // 100 years
static const int64 nTargetSpacing = 30; // MyCoin2: 30 sec
...
{
		if (pindexLast->nHeight+1 < 3153600000) 
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MyCoin2: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MyCoin2: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xeb4ac0ee05395f1ab3e8bd64fb01ddd9d583374047c318a9d445d8b31281afac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "MyCoin2 created by Ray!Connect me 1626907@gmail.com";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
    ...
    txNew.vout[0].nValue = 100 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1396761946; //MyCoin2 1395398354;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 392663416; //2084808754; //2084524493;

        if (fTestNet)
        {
            block.nTime    = 1396761946;
            block.nNonce   = 392663416;
        }
    ...
    assert(block.hashMerkleRoot == uint256("0x1b639079ffcd0b7a78246c0ce516e20d787fffad848a3bde3ebfd6ee98216280"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0x14, 0x92, 0x00 }; // MyCoin2
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MyCoin2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MyCoin2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MyCoin2Miner(CWallet *pwallet)
{
    printf("MyCoin2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MyCoin2-miner");
    ...
    try { loop {
        ...
        printf("Running MyCoin2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MyCoin2Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MyCoin2Miner, pwallet));
}
